...
from purchase_returns_repository import PurchaseReturnRepository
...
class PurchaseReturnService:
    ...
    def delete_purchase(self, purchase_id, user_id):
        ...
        # Handle financial reversals for Return
        if payment_status == 'completed':
            # Reverse cash drawer operation: money was added IN, now take it OUT
            notes = f"Reversal for purchase return deletion {purchase_id}"
            if not self.repository.add_cash_drawer_operation(purchase_total, 'Out', user_id, notes):
                raise Exception('Failed to reverse cash drawer operation')
        elif payment_status == 'pending' and supplier_id:
            # Add back to supplier balance (reverse the on-account deduction)
            # In return, liability was DECREASED, so to undo, we INCREASE it back.
            self.repository.update_supplier_balance_on_account(purchase_total, supplier_id)
    ...
    def _create_new_purchase(self, data, rows):
        ...
        last_purchase_id = self.repository.create_purchase(
            ...
            invoice_number, data['created_at'], data['payment_status'],
            data.get('user', {}).get('user_id'), currency_id,
    ...
    def _handle_payment_logic(self, purchase_id, invoice_number, is_update, **data):
        ...
        if supplier_id and payment_status == 'pending':
            # Decrement supplier liability for Return
            self.repository.decrease_supplier_balance_on_account(final_total, supplier_id, data.get('currency_id', 1))
            self.repository.insert_purchase_payment_on_account(
                purchase_id, invoice_number, final_total, purchase_date, created_at, supplier_id, 'Return pending - On Account'
            )
        else:
