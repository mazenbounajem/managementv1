...
class PurchaseReturnRepository:
    ...
    def get_max_purchase_id(self):
        try:
            max_id_result = []
            connection.contogetrows("SELECT MAX(id) FROM purchase_returns", max_id_result)
    ...
    def get_purchase_items(self, purchase_id):
        ...
        query = (
            ...
            "pi.unit_cost, pi.total_cost, pi.unit_price, pi.profit, ISNULL(pi.vat_amount, 0), ISNULL(pi.vat_percentage, 0) FROM purchase_return_items pi "
            "INNER JOIN products pr ON pr.id = pi.product_id WHERE pi.purchase_return_id = ?"
        )
    ...
    def get_all_purchases(self):
        raw_purchases_data = []
        connection.contogetrows(
            "SELECT p.id, p.purchase_date, s.name, p.total_amount, p.invoice_number, p.payment_status, p.account_code, p.auxiliary_number, c.currency_name FROM purchase_returns p INNER JOIN suppliers s ON s.id = p.supplier_id LEFT JOIN currencies c ON c.id = p.currency_id ORDER BY p.id DESC",
            raw_purchases_data
        )
    ...
    def get_purchase_details_for_deletion(self, purchase_id):
        purchase_data = []
        connection.contogetrows(
            "SELECT total_amount, payment_status, supplier_id FROM purchase_returns WHERE id = ?",
            purchase_data, params=[purchase_id]
        )
        return purchase_data

    def delete_purchase_items(self, purchase_id):
        connection.deleterow("DELETE FROM purchase_return_items WHERE purchase_return_id = ?", [purchase_id])

    def delete_purchase(self, purchase_id):
        connection.deleterow("DELETE FROM purchase_returns WHERE id = ?", [purchase_id])
    ...
    def insert_purchase_item(self, purchase_id, barcode, product_id, quantity, cost, subtotal, price, profit, discount, vat_amount=0, vat_percentage=0):
        purchase_item_sql = """
            INSERT INTO purchase_return_items (purchase_return_id, barcode, product_id, quantity, unit_cost, total_cost, unit_price, profit, discount_amount, vat_amount, vat_percentage)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
    ...
    def get_old_purchase_items(self, purchase_id):
        old_items = []
        connection.contogetrows("SELECT product_id, quantity FROM purchase_return_items WHERE purchase_return_id = ?", old_items, params=[purchase_id])
        return old_items
    ...
    def get_previous_purchase_financials(self, purchase_id):
        previous_data = []
        connection.contogetrows("SELECT payment_status, total_amount, supplier_id FROM purchase_returns WHERE id = ?", previous_data, params=[purchase_id])
    ...
    def delete_purchase_payment_records(self, purchase_id):
        connection.deleterow("DELETE FROM purchase_payment WHERE purchase_return_id = ?", [purchase_id])

    def get_invoice_number(self, purchase_id):
        return connection.getid("SELECT invoice_number FROM purchase_returns WHERE id = ?", [purchase_id])
    ...
    def update_supplier_balance_on_account(self, amount, supplier_id, currency_id=1):
    ...
    def decrease_supplier_balance_on_account(self, amount, supplier_id, currency_id=1):
        symbol = self.get_currency_symbol_by_id(currency_id)
        if symbol == '$':  # USD
            connection.update_supplierbalance("UPDATE suppliers SET balance_usd = balance_usd - ? WHERE id = ?", (amount, supplier_id))
        else:  # LL or other
    ...
    def insert_purchase_payment_on_account(self, purchase_id, invoice_number, total_amount, purchase_date, created_date, supplier_id, notes):
        purchase_payment_sql = """
            INSERT INTO purchase_payment (purchase_return_id, purchase_invoice_number, total_amount, purchase_date, created_date, supplier_id, status, amount_paid, payment_method, debit, credit, notes, purchase_id)
            VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?)
        """
        purchase_payment_values = (purchase_id, invoice_number, total_amount, purchase_date, created_date, supplier_id, 0, 'On Account', total_amount, 0, notes, None)
    ...
    def insert_purchase_payment_cash(self, purchase_id, invoice_number, total_amount, purchase_date, created_date, supplier_id, notes):
        purchase_payment_sql = """
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'completed', ?, ?, ?, ?, ?, ?)
        """
        purchase_payment_values = (purchase_id, invoice_number, total_amount, purchase_date, created_date, supplier_id, total_amount, 'Cash', total_amount, total_amount, notes, None)
        connection.insertingtodatabase(purchase_payment_sql, purchase_payment_values)

    def create_purchase(self, purchase_date, supplier_id, subtotal, discount_amount, final_total, invoice_number, created_at, payment_status, user_id=None, currency_id=1, total_vat=0):
        purchase_sql = """
            INSERT INTO purchase_returns (purchase_date, supplier_id, subtotal, discount_amount, total_amount, invoice_number, created_at, payment_status, user_id, currency_id, total_vat)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        purchase_values = (
            purchase_date, supplier_id, subtotal, discount_amount,
            final_total, invoice_number, created_at, payment_status, user_id, currency_id, total_vat
        )
    ...
    def update_purchase(self, purchase_date, supplier_id, subtotal, discount_amount, final_total, created_at, payment_status, purchase_id, user_id=None, currency_id=1, total_vat=0):
        purchase_sql = """
            UPDATE purchase_returns
            SET purchase_date = ?, supplier_id = ?, subtotal = ?, discount_amount = ?,
                total_amount = ?, created_at = ?, payment_status = ?, user_id = ?, currency_id = ?, total_vat = ?
            WHERE id = ?
        """
        purchase_values = (
            purchase_date, supplier_id, subtotal, discount_amount,
            final_total, created_at, payment_status, user_id, currency_id, total_vat, purchase_id
        )
        connection.insertingtodatabase(purchase_sql, purchase_values)
    ...
    def get_existing_invoices(self):
        existing_invoices = connection.contogetrows(
            "SELECT invoice_number FROM purchase_returns WHERE invoice_number IS NOT NULL", []
        )
    ...
    def get_purchase_data_for_invoice(self, purchase_id):
        purchase_data = []
        connection.contogetrows(
            "SELECT p.invoice_number, p.purchase_date, s.name, s.phone, p.total_amount, p.subtotal, p.currency_id, ISNULL(p.total_vat, 0) FROM purchase_returns p INNER JOIN suppliers s ON s.id = p.supplier_id WHERE p.id = ?",
            purchase_data, params=[purchase_id]
        )
        return purchase_data
    ...
    def get_purchase_items_for_invoice(self, purchase_id):
        purchase_items_data = []
        connection.contogetrows(
            "SELECT pr.product_name, pi.quantity, pi.unit_cost, pi.discount_amount, pi.total_cost, ISNULL(pi.vat_amount, 0) FROM purchase_return_items pi INNER JOIN products pr ON pr.id = pi.product_id WHERE pi.purchase_return_id = ?",
    ...
    def update_purchase_auxiliary_id(self, purchase_id, auxiliary_id):
        connection.insertingtodatabase("UPDATE purchase_returns SET auxiliary_id = ? WHERE id = ?", (auxiliary_id, purchase_id))

    def update_purchase_auxiliary_number(self, purchase_id, auxiliary_number):
        connection.insertingtodatabase("UPDATE purchase_returns SET auxiliary_number = ? WHERE id = ?", (auxiliary_number, purchase_id))
