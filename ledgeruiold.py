...
class LedgerUI:
    ...
    def build_account_tree(self, accounts, auxiliaries=[]):
        ...
        for aux in auxiliaries:
            ...
            if parent_num:
                if parent_num not in nodes:
                    ...
                    virt_name = ledger_name_by_code.get(parent_num, '') or 'Virtual Parent'
                    nodes[parent_num] = {
                        'id': f"V_{parent_num}",
                        'label': f"🏮 {parent_num} - {virt_name}",
                        'account_number': parent_num,
                        'children': [],
                        'type': 'ledger',
                        'data': {
                            'Id': 0,
                            'AccountNumber': parent_num,
                            'Name_en': virt_name,
                            'type': 'ledger'
                        }
                    }
        ...
        for code in sorted(nodes.keys(), key=lambda x: len(x)):
            ...
            if not found_parent:
                # Orphan account: attach as root-level.
                # Default behavior only keeps single-digit roots (1..7).
                # When show_all_roots is enabled, also show other discovered roots.
                    ...
                    with ModernCard(glass=True).classes('w-full p-6'):
                        with ui.row().classes('w-full justify-between items-center mb-4'):
                            ...
                            with ui.row().classes('gap-2 items-center'):
                                ...
                                ui.checkbox('All Roots', value=self.show_all_roots).props('dark').classes('ml-2').on('update:model-value', lambda e: setattr(self, 'show_all_roots', bool(e.value)))
