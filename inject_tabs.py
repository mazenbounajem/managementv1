...
def main():
    ...
    for i, line in enumerate(lines):
        if "def create_ui(self):" in line:
            start_idx = i
        if start_idx != -1 and "async def handle_sales_aggrid_click_wrapper(self, e):" in line:
            end_idx = i
            break
    ...
    if start_idx != -1 and end_idx != -1:
        ...
        for i in range(len(old_content)):
            ...
            with ui.tabs().classes('w-full bg-black/20 text-white border-b border-white/10 h-12') as tabs:
                op_tab = ui.tab('Sales Operations', icon='shopping_cart').classes('font-bold tracking-widest')
                ret_tab = ui.tab('Sales Returns', icon='assignment_return').classes('font-bold tracking-widest')
...
''')
        ...
        new_content.append('''                with ui.tab_panel(ret_tab).classes('w-full h-full p-0 m-0 overflow-hidden'):
                    try:
                        from sales_returns_ui import SalesReturnsUI
                        SalesReturnsUI(standalone=False)
                    except Exception as e:
                        from nicegui import ui
                        ui.label(f'Error loading Returns UI: {e}').classes('text-red-500 font-bold p-8')
''')
