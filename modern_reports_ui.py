...
from print_helper import add_print_css, print_pdf
import asyncio
import json
...
class ModernReportsUI:
    ...
    def create_ui(self):
        ...
        # Add global styles, print CSS, and Chart.js
    ...
    def export_pdf(self):
        ...
        try:
            headers = [col['headerName'] for col in self.data_table.options.get('columnDefs', [])]
            pdf_data = Reports.generate_pdf(
                f'{self.current_report_type.title()} Report',
                headers,
                self.report_data
            )
    ...
    def print_report(self):
        """Print the current report via PDF generation"""
        try:
            if not self.report_data:
                ModernToast.show('No data to print', 'warning')
                return
            headers = [col['headerName'] for col in self.data_table.options.get('columnDefs', [])]
            pdf_data = Reports.generate_pdf(
                f'{self.current_report_type.title()} Report',
            ...
            print_pdf(pdf_data,
                      f'report_{self.current_report_type}_{datetime.now().strftime("%Y%m%d")}.pdf',
                      f'{self.current_report_type.title()} Report')
        except Exception as e:
            ModernToast.show(f'Print failed: {str(e)}', 'error')
